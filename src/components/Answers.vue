...
export default {
    ...
    computed:{
        ...
        correctchoice: function(){
            var correctAnswers = this.selected.map(sel => this.answers.find(answer => answer.text === sel)).filter(answer => answer != undefined && answer.correct_choice==='true');
        ...
        nrcorrectanswers: function(){
            return this.answers.filter(answer => answer.correct_choice=='true').length;
        },
        // returns if task solved correctly
        correctlysolved: function(){
            if(this.selected.length === this.nrcorrectanswers && this.correctchoice.length === this.nrcorrectanswers)
                return true
            else
                return false;
        }
    ...
    methods:{
        ...
        highlightChosen(){
            var chosenCheck = document.querySelectorAll('[aria-checked="true"]');
            var color;
            var tagName = 'li';
            var i;
            for(i = 0; i < chosenCheck.length; i++){
                var element = chosenCheck[i];
