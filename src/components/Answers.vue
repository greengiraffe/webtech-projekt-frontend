...
export default {
    ...
    // checks whether a task is of type text or choice
        ...
        correctchoice: function () {
            const correctAnswers = this.selected.map(sel => this.answers.find(answer => answer.text === sel)).filter(answer => answer !== undefined && answer.correct_choice === 'true')
        ...
        correctlysolved: function () {
            if (this.selected.length === this.nrcorrectanswers && this.correctchoice.length === this.nrcorrectanswers) { return true } else { return false }
        }
    },
    methods: {
        verify () {
